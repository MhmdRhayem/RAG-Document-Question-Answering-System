    ...
    class MyEmbedding:
        def __init__(self):
            self.model = SentenceTransformer(
                "all-MiniLM-L6-v2", trust_remote_code=True, truncate_dim=384
            )
...
def ollama_embedding():
    class OllamaEmbedding:
        ...
        def embed_documents(self, docs):
            embeddings = []
            for doc in docs:
                response = ollama.embeddings(model=self.model_name, prompt=doc)
...
# Vector Store
def create_vector_store(embedding_type):
...
def add_documents(vector_db, docs):
